...
function load() {
  ...
  console.log("draftPlayers length: " + draftPlayers.length);
...
function generateTeamFA() {
  ...
  if (!tradedAway.includes("Bryce Huff")) {
  ...
  //// PICTURE
    ...
    var img = document.createElement("img");
    img.setAttribute("src", BryceHuff.img);
  ...
  ////
    ...
    nameP.innerHTML = BryceHuff.name.toUpperCase();
    nameCol.appendChild(nameP);
    row.appendChild(nameCol);
    ...
    posP.innerHTML = BryceHuff.pos + "  /  Age: " + BryceHuff.age;
    ...
    priceP.innerHTML = BryceHuff.cYears + " yr./$" + BryceHuff.cTotal + "M";
    priceCol.appendChild(priceP);
    row.appendChild(priceCol);


    if (!offeredArr.includes(BryceHuff)) {
      ...
      signButton.addEventListener('click', function() {
        extendedArr.push(BryceHuff);
        offeredArr.push(BryceHuff);
        updateCapBar() // fix
        generateTeamFA();
        generateRoster();
      });
...
function getPick(team) {
  ...
  // }
  if (draftSummary.length == 1) {
    return draftPlayers[1];
  }
  // if (draftSummary.length == 1) {
  //   var temp = [CalebWilliams];
  //   var temp2 = [];
